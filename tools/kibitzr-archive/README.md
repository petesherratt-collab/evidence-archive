...
`archive report` turns an archive into a portable directory of linked,
read-only HTML pages:

```sh
kibitzr archive verify --root archive
...
- a failed fetch makes no document-change claim;
- the first successful normalisation is an **Initial capture**;
- **Raw changed** means the fetched response bytes changed;
- **Document changed** means the content selected before the reporting
  `changes` transform changed.

Only a genuine document change gets a prominent change page. Raw-only churn,
such as a rotating CSP nonce with identical selected content, is shown more
quietly. The report does not call changes important, minor, material or
fraudulent. It presents the observation bracket, before/after content, neutral
field changes, hashes, annotations and timestamp coverage so a reader can make
that judgement.

### How a displayed diff earns trust

The normalisation table stores a digest, not the selected bytes. For both sides
of a change, the generator loads the raw response through the chained
`content_sha256`, verifies the retained bytes, checks that the current rules
match the recorded transform fingerprint, reapplies only the document-producing
part of the pipeline, encodes it as the collector did, and requires its SHA-256
to equal the recorded normalisation digest. A semantic or textual diff appears
only if every check passes.

If historical rules are unavailable, a fingerprint changed, a raw blob is
missing or corrupt, transformation fails, or regenerated bytes do not match,
the evidence page remains useful but does not invent a diff. It shows the
recorded hashes, raw links, annotations, timestamp state and the exact reason
verified reconstruction is unavailable. Current rules cannot reconstruct a
historical transform revision unless those exact rules are still available.

Timestamp coverage follows the same trust path as `fsck`: proof bytes → the
manifest bytes they cover → manifest heads → chains recomputed from
`polls.db`. The mutable `anchor` table is used only for its pending/complete
status label, never as the source of the covered head. Pending calendar proofs
are not described as Bitcoin-attested.

The report is a **derived view, not evidence itself**. A third party should use
the commands printed in it to verify the source archive and OpenTimestamps
...
it, the command looks only for `kibitzr.yml` in the current directory. Do not
use a public label or publish files until the intended disclosure scope has
been reviewed.

> **Privacy and publication warning:** raw responses may contain copyrighted
> or personal material; exception strings can expose machine or network
